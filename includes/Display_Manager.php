...
class Display_Manager {
	...
	//SELECT * FROM `ffi_aam_audiocache` WHERE `Date` > '2013-01-01' AND `Date` < '2013-12-31' ORDER BY YEAR(`Date`) DESC, MONTH(`Date`) DESC, `Date` ASC;
	...
	//The main page (the current year) will display a different set of images than previous years
		if ($this->year == $this->dateFormatter->format("Y")) {
			...
			$background = array("earbuds.jpg", "sound-board.jpg");
			$rand = mt_rand(0, 1);
		}
